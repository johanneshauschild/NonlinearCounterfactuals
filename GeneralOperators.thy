...
definition (in preordered_counterfactual_structure) general_would ::
  ...
  where
    \<open>\<phi> \<box>\<rightarrow>\<hungarumlaut> \<psi> \<equiv> {w. (\<forall> w1. w \<le><w> w1 \<longrightarrow>  w1 \<notin> \<phi>) \<or> (\<forall> w1. w \<le><w> w1 \<and> w1 \<in> \<phi> \<longrightarrow> 
...
abbreviation (in preordered_counterfactual_structure) general_more_possible ::
  \<open>'i set \<Rightarrow> 'i set \<Rightarrow> 'i set\<close> (infixr\<open>\<prec>\<hungarumlaut>\<close> 100)  
  where 
...
abbreviation (in preordered_counterfactual_structure) general_at_least_as_possible :: 
  ...
  where 
    \<open>\<phi> \<preceq>\<hungarumlaut> \<psi> \<equiv> (\<phi> \<union> \<psi>) \<diamond>\<Rightarrow>\<hungarumlaut> \<phi>\<close>
...
lemma (in preordered_counterfactual_structure) general_might_follows_definition:
  shows \<open>w \<in> \<phi> \<diamond>\<rightarrow>\<hungarumlaut> \<psi> \<longleftrightarrow> w \<in> {w. \<exists> w1. w \<le><w> w1 \<and> w1 \<in> \<phi> \<and> 
...
next
  assume \<open>w \<in> {w. (\<exists> w1. w \<le><w> w1 \<and> w1 \<in> \<phi>) \<and> (\<forall> w1. w \<le><w> w1 \<and> w1 \<in> \<phi> \<union> \<psi> \<longrightarrow> 
          (\<exists> w2. w2 \<le><w> w1 \<and> w2 \<in> \<phi> \<and> (\<forall> w3. w3 \<le><w> w2 \<longrightarrow> w3 \<notin> \<psi>)))}\<close>
  hence \<open>w \<in> {w. (\<exists> w1. w \<le><w> w1 \<and> w1 \<in> \<phi> \<union> \<psi>) \<and> (\<forall> w1. w \<le><w> w1 \<and> w1 \<in> \<phi> \<union> \<psi> \<longrightarrow>
         (\<exists> w2. w2 \<le><w> w1 \<and> w2 \<in> \<phi> \<union> \<psi> \<and> (\<forall> w3. w3 \<le><w> w2 \<longrightarrow> 
         w3 \<in> UNIV - (\<phi> \<union> \<psi>) \<union> (UNIV - \<psi>))))}\<close> using subset_can_be_generalized by auto
  thus \<open>w \<in> (\<diamond> (\<phi> \<union> \<psi>)) \<inter> ((\<phi> \<union> \<psi>) \<box>\<rightarrow>\<hungarumlaut> (UNIV - \<psi>))\<close> 
...
lemma (in preordered_counterfactual_structure) general_at_least_as_possible_follows_definition:
  shows 
    ...
    (\<exists> w1. w \<le><w> w1 \<and> (w1 \<in> \<phi> \<union> \<psi>) \<and> (\<forall> w2. w2 \<le><w> w1 \<and> w2 \<in> \<psi> \<longrightarrow> 
...
next
  have  \<open>i3 \<in> {w. \<exists> w1. w \<le><w> w1 \<and> w1 \<in> \<phi> \<and> (\<forall> w2. w2 \<le><w> w1 \<and> w2 \<in> \<phi> \<longrightarrow> 
